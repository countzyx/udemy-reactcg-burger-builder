...
class ContactData extends Component<Props, State> {
  ...
  inputChangedHandler = (event: SyntheticEvent<HTMLInputElement>, id: string) => {
    ...
    const formIsValid = Object.keys(updatedOrderForm).reduce<boolean>((acc, k) => {
      console.log(updatedOrderForm[k]);
      return updatedOrderForm[k].valid === undefined ? acc : acc && updatedOrderForm[k].valid;
    }, true);
